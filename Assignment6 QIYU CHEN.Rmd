...
```
We choose grade and assignment, since we find that when we add forum.posts to the model, it will not increase the accuracy. And if we use these two variable we will get the lowest error and the model will not be so complex.
...
```
Accoring to the function "printcp" above, we find that when cp = 0.058182 we will get two nodes.
...
```
Let's calculate the error rate. Tree 1 is 7814/n and tree 2 is 4637/n. So Tree 2 has a lower error rate.