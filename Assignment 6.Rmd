...
#the following variables were used- a) grade and b) assignemnt
#first load the library rpart
library(rpart)
...
#Next, calculate an error rate of the original tree
#There are 10000 students (add yes respones and divide by n)
(24+7790)/10000  
#=.78
```

```{r}
#Calculation of an error rate of the pruned tree (do the same for the pruned tree)
(1184+3453)/10000
#=.46

#From the previous calculations, the pruned tree has the lower error rate (approximately 46%). It can be argued that the pruned tree is more "generalizable" than the original tree. 
```
