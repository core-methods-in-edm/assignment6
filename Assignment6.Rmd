...
post(c.tree1, file = "tree2.ps", title = "Students paid for the class or not")
...
### everytime we add a node, we add a cost to it
### so we prune it to the 3th branch

c.tree2 <- prune(c.tree1, cp = 0.0036765)

#Visualize this tree and compare it to the one you generated earlier

post(c.tree2, file = "tree2a.ps", title = "MOOC") #This creates a pdf image of the tree
```
...
```
Compute 2 new dummy variables to see whether our prediction is correct:

```{r}

M2$error1 <- ifelse(M2$certified == M2$predict1, 1, 0)
M2$error2 <- ifelse(M2$certified == M2$predict2, 1, 0)
...
So the correct prediction rate for the prediction1 is 0.6672
and the correct prediction rate for the prediction 2 is 0.7575

so the second tree gives better prediction than the first one. 
