...
c.tree1 <- rpart(certified ~ forum.posts + grade + assignment, method="class", data=M1, control = rpart.control(minsplit = 10))
...
```{r}
c.tree2 <- prune(c.tree1, cp = printcp(c.tree1)[2,1]) #Set cp to the level at which you want the tree to end
...
acc1 <- sum(t1[1,1], t1[2,2])/sum(t1)
acc1
acc2 <- sum(t2[1,1], t2[2,2])/sum(t2)
acc2

#The pruned tree has a lower error rate as 53% of its certification predictions are accurate compared to the original tree(21%).
```
