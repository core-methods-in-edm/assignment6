...
#Plot your tree
post(c.tree1, file = "tree1.pdf", title = "MOOC") #This creates a pdf image of the tree
...
err1 <- (1-((table1[1,1]+table1[2,2])/sum(table1)))*100
err2 <- (1-((table2[1,1]+table2[2,2])/sum(table2)))*100

# The second tree, which has two nodes, has a lower error rate. 
```
...
```{r}
N1 <- read.csv("student.record.csv", header = TRUE)
N2 <- read.csv("student.course.csv", header = TRUE)

N3 <- merge(N1, N2, by.x = "ANONID", all.x=TRUE)
c.tree3 <- rpart(certified ~ ., method = "class", data = N3)
