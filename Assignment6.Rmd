...
#In the first model, SAT scores, ACT scores and sex to predict GPA
c.tree3 <- rpart(as.factor(HSGPA) ~., method = "class", data = data)
printcp(c.tree3)

#In the second model, composite SAT scores, composite ACT scores and sex to predict GPA
HSGPA <- data[,1]
SEX <- data[, 10]
ACT <- data[, 2:6]
...
#For first model the accuracy is 26.08%, the accuracy of the second model is 26.04%. Second model uses fewer variables to predict the outcome, and both accuracy are close.
