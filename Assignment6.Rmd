...
a<-table(M2$certified, M2$predict1)

b<-table(M2$certified, M2$predict2)
sum(diag(a))/sum(a)
...
```{r}
library(readr)
d1<-read_csv("student.record.csv")
d2 <- d1[,c(3:11,13)]
d3 <- na.omit(d2)
#model 1
d.tree1 <- rpart(as.factor(MAJOR1_DESCR) ~ .,  method="class", data=d3)
printcp(d.tree1)
post(d.tree1, file = "tree3.ps", title = "MOOC")
p1<-predict(d.tree1, d3, type = "class")
t1<-table(d3$MAJOR1_DESCR, p1)
sum(diag(t1 ))/sum(t1 )
#model 2
d4 <- d3[,-1]
d.tree2 <- rpart(SEX ~ .,  method="class", data=d4)
printcp(d.tree2)
post(d.tree2, file = "tree4.ps", title = "MOOC") 
p2<-predict(d.tree2, d4, type = "class")
t2<-table(d4$SEX, p2)
sum(diag(t2))/sum(t2)
#the second model is much better
```
