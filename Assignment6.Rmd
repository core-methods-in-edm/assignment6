...
# I used variable grade and assignment because forum.posts did not influence accuracy.

c.tree1 <- rpart(as.factor(certified) ~ grade + assignment,  method="class", data=M1)
c.tree2 <- rpart(as.factor(certified) ~ forum.posts + grade + assignment,  method="class", data=M1)
...
post(c.tree2, file = "tree1.ps", title = "MOOC")
```
...
it looks like the pruned tree has a lower error rate by comparing error rate.
