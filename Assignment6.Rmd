...
#According to these two tables displayed below, the first one’s number of correction is 2056+130=2186, which is smaller than the second one (896+4467=5363) while the totals are both 10,000. In this way, the second tree has a lower error rate.
...
#the data set is too big (I got an error if I use M3-Error: vector memory exhausted (limit reached?). So I decided to select some data)

library(dplyr)
M4 <- select (M3,"ANONID","GPAO","GRD_PTS_PER_UNIT")

#c.tree3 <- rpart(GPAO~ANONID+GRD_PTS_PER_UNIT,method = "class",data = M4)
