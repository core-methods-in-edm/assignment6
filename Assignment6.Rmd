...
```{r}
c.tree2 <- prune(c.tree1, cp = 0.01000)#Set cp to the level at which you want the tree to end
...
The error rate on each tree is the same since the same number of students were divided into each branch. I guess we chose variables (forum.post + grade) that were effective in informing the model. The tables below show the same numbers for each value of the table.
