...
c.tree1 <- rpart(certified ~ grade + forum.posts + grade + assignment, method="class" , data = M1, control=rpart.control(minsplit=1,minbucket=1,cp=0.001))
...
```{r}
c.tree2 <- prune(c.tree1, cp = 0.0032680)
...
```
#We can add the yes/no and no/yes from the table and divide by the total to see which has more accurate prediction. The pruned tree does a better job at predicting as the error was 24.42% vs. 33.29%
