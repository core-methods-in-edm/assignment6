...
myFormula<- certified~forum.posts + grade + assignment
c.tree1 <- rpart(myFormula, data=M1, control=rpart.control(minsplit=10))
...
```{r}
df1<-as.data.frame(printcp(c.tree1))

c.tree2 <- prune(c.tree1, cp = df1[2,1])#Set cp to the level at which you want the tree to end
